...
def my_ground_entity_label(statement_path, cpnet_vocab_path, pattern_path, output_path,
              num_processes=1, debug=False):
    global PATTERN_PATH, CPNET_VOCAB
    if PATTERN_PATH is None:
        PATTERN_PATH = pattern_path
        CPNET_VOCAB = load_cpnet_vocab(cpnet_vocab_path)
    ...
    for line in lines:
        ...
        statements.append(j)
        if n_choice is None:
            n_choice = len(j['statements'])
        for statement, answer in zip(j["statements"], j["question"]["choices"]):
            sents.append(statement["statement"])
            ans = answer['text']
            # ans = " ".join(answer['text'].split("_"))
            try:
                ...
                print(ans)
            if statement['label']:
                true_sents.append(statement['statement'])
                true_answers.append(ans)
            answers.append(ans)
    ...
    for i in range(len(res)):
        pos = i // n_choice
        res[i]['sent'] = true_sents[pos]
        res[i]['ans'] = true_answers[pos]
...
def my_ground_text_label(statement_path, cpnet_vocab_path, pattern_path, output_path,
                         num_processes=1, debug=False):
    global PATTERN_PATH, CPNET_VOCAB
    if PATTERN_PATH is None:
        PATTERN_PATH = pattern_path
    ...
    sents_for_input = []
    answers_for_input = []
    for sent, ans in zip(true_sents, true_answers):
        sents_for_input += [sent] * n_choice
        answers_for_input += [ans] * n_choice

    res = match_mentioned_concepts(sents_for_input, answers_for_input, num_processes)
    res = prune(res, cpnet_vocab_path)
    for i in range(len(res)):
        res[i]['sent'] = sents[i]
        res[i]['ans'] = answers[i]
...
if __name__ == "__main__":
    ...
    # my_ground_text_label("../data/csqa/statement/dev.statement.jsonl",
