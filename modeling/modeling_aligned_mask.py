...
from transformers import modeling_roberta, RobertaTokenizer
...
class MaskedGreaseLM(nn.Module):
    ...
    def __init__(self, args={}, model_name="roberta-large", k=5, n_ntype=4, n_etype=38,
        ...
        super().__init__()
        self.lmgnn = MaskedLMGNN(args, model_name, k, n_ntype, n_etype,
    ...
    def forward(self, *inputs, cache_output=False, detail=False, mini_batch_id=None, purpose=None, labels=None,
        ...
        *lm_inputs, concept_ids, node_type_ids, node_scores, adj_lengths, special_nodes_mask, edge_index, edge_type = _inputs
        ...
        # print(node_type_ids, torch.max(node_type_ids, dim=-1))
        # exit('debug')
        linked_span = []
        ...
        for _ in align_data[1]:
            linked_ids += _
        masked_loss = self.lmgnn(lm_inputs, concept_ids,
                                 node_type_ids, node_scores, adj_lengths, special_nodes_mask, adj,
                                 emb_data=None, purpose=purpose,
                                 align_data=(linked_span, linked_ids),
                                 num_choice=nc)
...
class MaskedLMGNN(nn.Module):
    ...
    def __init__(self, args={}, model_name="roberta-large", k=5, n_ntype=4, n_etype=38,
        ...
        self.lm_mask_pooler = layers.AlignMaskPool(mean=args.use_mean_pool)
        self.gnn_mask_pooler = layers.AlignMaskPool(mean=args.use_mean_pool)

        self.align_mask = args.align_mask
        self.align_option = args.align_option
        self.is_med = args.dataset == 'medqa_usmle'
        lm_dim = 1024 if not self.is_med else 768
        if self.align_option == 'contrastive-cls':
            self.align_predictor = layers.AlignClassifier(lm_dim + 200)
        elif self.align_option == 'semantic-mse':
            self.align_predictor = layers.AlignSemantic(lm_dim, 200, loss_type='mse')
        elif self.align_option == 'semantic-cos':
            self.align_predictor = layers.AlignSemantic(lm_dim, 200, loss_type='cosine')
        elif self.align_option == 'no-align':
            assert args.mlm
        elif self.align_option == 'mutual-info':
            self.align_predictor = layers.AlignMutualInfo(lm_dim, 200, 512)
        else:
            raise KeyError

        self.negative_strategy = args.negative_strategy
        self.span_num = args.span_num
        self.neg_num = args.neg_num
        self.mix_neg = args.mix_neg

        self.mlm = args.mlm
        self.mlm_mask_percent = args.mlm_mask_percent
        if self.mlm:
            self.mlm_predictor = layers.MaskedTokenCls(lm_dim)
    ...
    def forward(self, inputs, concept_ids, node_type_ids, node_scores, adj_lengths,
                special_nodes_mask, adj, emb_data=None, purpose=None, align_data=None,
                num_choice=None):
        # LM inputs
        ...
        assert self.align_mask and align_data is not None
        ...
        for _ in range(self.span_num):
            ...
            for spans, entity_ids in zip(linked_span, linked_entity_ids):
                if len(spans):
                    ...
                    chosen_spans.append([-1, -1])
                    chosen_entity_ids.append([])
        ...
        if self.negative_strategy == 'random':
            ...
            # print(torch.sum(negative_gnn_mask, dim=-1))

        original_lm_ids = None
        if self.mlm:
            mlm_mask = torch.zeros(batch_size, seq_length)
            mlm_mask = mlm_mask.bernoulli_(self.mlm_mask_percent).long()
            for partial_lm_mask in lm_mask:
                mlm_mask[partial_lm_mask] = 0
            mlm_mask = mlm_mask == 1
            original_lm_ids = input_ids[mlm_mask]
            if not self.is_med:
                input_ids[mlm_mask] = 50264
            else:
        ...
        # Merged core
        masked_outputs, (masked_gnn_output, *_) = self.mp(input_ids, token_type_ids, attention_mask,
        ...
        if self.align_option == 'no-align':
            assert self.mlm
            assert original_lm_ids is not None
            lm_masked_embeds = masked_outputs[0][mlm_mask]
            # print(lm_masked_embeds.shape, original_lm_ids.shape)
            lm_masked_pred_loss = self.mlm_predictor(lm_masked_embeds, original_lm_ids)

            align_loss = lm_masked_pred_loss
        else:
            all_lm_embeds = []
            all_gnn_embeds = []
            all_negative_embeds = []
            lm_seq_outputs = masked_outputs[0]
            if self.negative_strategy == 'random':
                for lm_mask_partial, gnn_mask_partial, negative_gnn_mask_partial in zip(
                    ...
                    for i, (_lm_mask, _gnn_mask) in enumerate(zip(lm_mask_partial,
                        ...
                        if _lm.shape[0] == 0 or _gnn.shape[0] == 0 or flag:
                            continue
                        # print(_lm.shape, _gnn.shape, _neg_gnn.shape)
            ...
            elif self.negative_strategy == 'in-batch':
                exit('buggy')
            else:
                raise KeyError
            ...
            if self.align_option == 'contrastive-cls':
                if self.negative_strategy == 'random':
                    ...
                    positive_sample = torch.cat((lm_masked_embeds, gnn_masked_embeds), dim=-1)
                    negative_sample = []
                    for lm_emb, neg_embs in zip(lm_masked_embeds, negative_gnn_embeds):
                        for _emb in neg_embs:
                            negative_sample.append(torch.cat((lm_emb, _emb), dim=-1))
                    negative_sample = torch.stack(negative_sample, dim=0)
                    # print(positive_sample.shape, negative_sample.shape)
                else:
                    raise KeyError
            ...
            elif self.align_option == 'semantic-mse' or self.align_option == 'semantic-cos':
                align_loss = self.align_predictor(lm_masked_embeds, gnn_masked_embeds, negative_gnn_embeds)
            elif self.align_option == 'mutual-info':
                align_loss = self.align_predictor(lm_masked_embeds, gnn_masked_embeds, negative_gnn_embeds)
            else:
                raise KeyError
...
class TextKGMessagePassing(ModelClass):
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, special_tokens_mask, H, A,
                ...
                head_mask=None, output_hidden_states=True, output_edge_embeddings=False, lm_only=False):
        """
...
class RoBERTaGAT(modeling_bert.BertEncoder):
    ...
    def forward(self, hidden_states, attention_mask, special_tokens_mask,
                ...
                output_edge_embeddings=False, lm_only=False):
        """
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if not lm_only and i >= self.num_hidden_layers - self.k:
                if self.iterative_ie_layer:
                    ...
                    if self.info_exchange == True or (self.info_exchange == "every-other-layer" and (
                            i - self.num_hidden_layers + self.k) % 2 == 0):
